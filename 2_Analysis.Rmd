...
#Create csv dataframe from LCP Circuitscape outputs
  ...
  #######################################
    LCP_2011 <- sf::st_read(paste0(files, "KF_2011/output/link_maps.gdb"), layer = "KF_2011_LCPs") %>% st_set_geometry(NULL)
    LCP_2015 <- sf::st_read(paste0(files, "KF_2015/output/link_maps.gdb"), layer = "KF_2015_LCPs") %>% st_set_geometry(NULL)
    LCP_2017 <- sf::st_read(paste0(files, "KF_2017/output/link_maps.gdb"), layer = "KF_2017_LCPs") %>% st_set_geometry(NULL)
  ...
  #Baseline eq1
  #######################################
    LCP_2011_beq1 <- sf::st_read(paste0(files, "KF_2011_base_eq1/output/link_maps.gdb"), layer = "KF_2011_base_eq1_LCPs") %>% st_set_geometry(NULL)
    LCP_2015_beq1 <- sf::st_read(paste0(files, "KF_2015_base_eq1/output/link_maps.gdb"), layer = "KF_2015_base_eq1_LCPs") %>% st_set_geometry(NULL)
    LCP_2017_beq1 <- sf::st_read(paste0(files, "KF_2017_base_eq1/output/link_maps.gdb"), layer = "KF_2017_base_eq1_LCPs") %>% st_set_geometry(NULL)
    LCP_2011_csv_beq1<-LCP_2011_beq1%>% dplyr::mutate(Year=2011) %>% select(Year, everything())
    LCP_2015_csv_beq1<-LCP_2015_beq1%>% dplyr::mutate(Year=2015)%>% select(Year, everything())
    LCP_2017_csv_beq1<-LCP_2017_beq1%>% dplyr::mutate(Year=2017)%>% select(Year, everything())
...
for (c in q){
  ...
  dir.create(paste0("Data/2_Analysis/Stats/", c, "/"), showWarnings = FALSE)  
    ...
    # Summarizations
    df_summary<-df_reg %>% group_by(Year) %>% summarize(LCP=n(), CWDmean=mean(CW_Dist_km), CWDsd=sd(CW_Dist_km),
...
models_df2<-models_df %>% dplyr::mutate(model="old2")
modelsdfs<-rbind(models_df, models_df2)
